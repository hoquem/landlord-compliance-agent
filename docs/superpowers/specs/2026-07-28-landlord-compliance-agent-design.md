...
1. **Per-property vs per-business HMRC reporting: RESOLVED 2026-07-29 — per-entity aggregation holds.** Design keeps per-property allocation internally and aggregates per entity for export. Research against the HMRC developer hub (Property Business API v6.0, MTD ITSA end-to-end service guide, `income-tax-mtd-changelog` SA105 mapping) confirms HMRC treats all of one owner's UK rental property as a **single UK property business** — quarterly figures are pooled across properties, submitted per income source, not per property. Per-property detail stays a supplementary sheet for the accountant. Task 16 proceeds as designed.
   - **Also confirmed:** submissions are **cumulative year-to-date** (replacing, not appending) for tax year 2025-26 onwards — matches what `core/quarters.py` already computes.
   - **Sourcing caveat:** the aggregation answer rests on HMRC service-guide wording ("for each self-employment and property income source") corroborated by a third-party MTD vendor's docs, rather than a single unambiguous primary statement. Strong but not airtight; revisit before iteration 2 builds direct filing.
   - **Two gaps found in our 15-category enum** (`core/categories.py`), both deliberately out of MVP scope — flagged, not fixed:
     - No equivalent for HMRC's `residentialFinancialCostsCarriedForward` (unused residential finance cost carried forward). Matters when finance costs exceed profit; the accountant handles carry-forward in the current export-pack model, but direct filing would need it.
     - No equivalent for `taxDeducted`, `premiumsOfLeaseGrant`, `reversePremiums`, or the rent-a-room fields. None apply to Mahmud's portfolio today; adding them without a real case would be speculative.
   - **API-shape caveat for iteration 2:** the field names in the SA105 mapping nest under `ukNonFhlProperty`, but furnished-holiday-letting status was abolished from April 2025, so the current API version may have restructured that branch. Irrelevant to the MVP (our CSV uses our own category enum); re-verify field paths when direct filing is built.
